...
\Cref{explicit_bias,unequal_treatment} can be merged in the definition of a \emph{comprehensive individual fairness}, as follows.
%
...
\begin{definition}[Group unfairness: explicit bias]\label{explicit_bias_group}
	...
	% \textbf{\textcolor{magenta}{ED: 
			% A decision for the individual $\boldsymbol{z}_1$ with non-protected attribute values $\boldsymbol{x}_{1} \in X$ and protected attribute values $s \in A$, is \emph{group unfair} with respect to $s$ if the counterfactual point of the factual instance $\boldsymbol{z}_{1}$ has different values for some sensitive variable $A$: 
			% $$ f_{e}^{h}(\boldsymbol{z}_{1}) = \boldsymbol{z}_{1}^{c} = (\boldsymbol{x}_{1}^{c}, \boldsymbol{a}_{1}^{c}),  \quad \boldsymbol{a}_{1} \neq \boldsymbol{a}_{1}^{c}, i.e., \boldsymbol{a}_{i}^{c} \neq s.$$
			% }}
\end{definition}
%
...
%This implies that the sets of actions that are needed for an individual to change class are not only based on the sensitive attributes but also vary on the non-protected attributes if we change the sensitive ones.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fairness Metrics Based on Counterfactuals}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Assume we have a dataset represented as $\{(\boldsymbol{x}_{1}, \boldsymbol{a}_{1}, y_{1}), \cdots , (\boldsymbol{x}_{n}, \boldsymbol{a}_{n}, y_{n})\}$ consisting of $n$ instances of $(X, A, Y)$. 
% 
%Let $f: Z \rightarrow \mathbb{R}$ denote a black-box classifier, where we denote by $\hat{y}=f(\boldsymbol{x}, \boldsymbol{a})$ the predicted outcome for a given input.
...
\end{equation*}
% 
%In order to study the unfairness of the classifier, we propose evaluating discrimination through the prism of counterfactual explanations.
